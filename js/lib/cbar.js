...
(function($){
	$.cookieBar = function(options,val){
		...
		var defaults = {
			...
			acceptOnScroll: true, //Set to true to silently accept cookies when visitor scrolls X pixels up or down
		...
		if(doReturn=='cookies'){
			...
			if(options.forceShow || cookieValue=='enabled' || cookieValue==''){
				if(options.append){
					$(options.element).append('<div id="cookie-bar"'+fixed+zindex+'><p>'+message+acceptButton+declineButton+'<br><br>'+policyButton+'</p></div>');
				}else{
					$(options.element).prepend('<div id="cookie-bar"'+fixed+zindex+'><p>'+message+acceptButton+declineButton+'<br><br>'+policyButton+'</p></div>');
				}
			}
			...
			if(!isNaN(options.acceptOnScroll)){
                ...
                if (cookieValue=="accepted" && !acceptednow) {
                  (function (i, s, o, g, r, a, m) {
                     i['GoogleAnalyticsObject'] = r; i[r] = i[r] || function () {
                       (i[r].q = i[r].q || []).push(arguments)
                     }, i[r].l = 1 * new Date(); a = s.createElement(o),
